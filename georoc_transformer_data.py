...
Args= {
    ...
    "num_visuals": 10,
    "num_workers": 4,
    "save_dir": "./save/train/baseline-35",
    "seed": 224,
...
#             batch_size, c_len, w_len = self.abstract_char_idxs.size()
#             ones = torch.ones((batch_size, 1), dtype=torch.int64)
#             self.abstract_idxs = torch.cat((ones, self.abstract_idxs), dim=1)
#             self.title_idxs = torch.cat((ones, self.abstract_idxs), dim=1)



#             ones = torch.ones((batch_size, 1, w_len), dtype=torch.int64)

#             self.abstract_char_idxs = torch.cat((ones, self.abstract_char_idxs), dim=1)
#             print("path in ssquat5")
#             self.title_char_idxs = torch.cat((ones, self.title_char_idxs), dim=1)
...
def build_features(args, examples, data_type, out_file, word2idx_dict, char2idx_dict, is_test=False):
    para_limit = 2000#args.test_para_limit if is_test else args.para_limit
    ...
    def drop_example(ex, is_test_=False):
        if is_test_:
            ...
            drop = len(ex["abstract_tokens"]) > para_limit or \
                   len(ex["title_tokens"]) > ques_limit 
    ...
    for n, example in tqdm(enumerate(examples)):
        ...
        abstract_idx = np.zeros([para_limit+150], dtype=np.int32)

        abstract_char_idx = np.zeros([para_limit, char_limit], dtype=np.int32)
      

        title_idx = np.zeros([40], dtype=np.int32)
        title_char_idx = np.zeros([para_limit, char_limit], dtype=np.int32)
        ...
        # print("char abs",abstract_char_idxs)
        # print("title",title_idxs)
        # print("char title",title_char_idxs)
...
def pre_process():
    ...
    # Preprocess dataset
    args_.train_file ="data/data_abstracts.json"#url_to_data_path(args_.train_url)
    args_.dev_file ="data/data_abstracts.json"#url_to_data_path(args_.train_url)
    ...
    train_file="/home/bahrami/Documents/exercise/Deeplearningtopics/qangaroo_v1.1/qangaroo_v1.1/wikihop/data_abstracts.json"
    dev_file="/home/bahrami/Documents/exercise/Deeplearningtopics/qangaroo_v1.1/qangaroo_v1.1/wikihop/data_abstracts.json"
